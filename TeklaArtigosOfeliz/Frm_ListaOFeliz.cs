...
namespace TeklaArtigosOfeliz
{
    public partial class Frm_ListaOFeliz : Form
    {
        ...
        public void carregadados()
        {
            ...
            try
            {
                using (StreamReader file = new StreamReader(@"c:\r\OFELIZ.CSV", Encoding.Default, true))
                {
                    while ((line = file.ReadLine()) != null)
                    {
                        var fields = line.Split(';');
                        ...
                        switch (i)
                        {
                            case 2: label1.Text = fields[1]; break;
                            case 3: label2.Text = fields[1]; break;
                            case 4: lbl_numeroobra.Text = fields[1]; break;
                            case 5: label4.Text = fields[1]; break;
                            case 6: label5.Text = fields[1]; break;
                        }
                ...
                for (int a = 0; a < dataGridView1.Rows.Count - 1; a++)
                {
                    for (int b = 0; b < dataGridView1.ColumnCount - 1; b++)
                    {
                        ...
                        {
                            var cellValue = dataGridView1.Rows[a].Cells[b].Value;
                            dataGridView1.Rows[a].Cells[b].Value = cellValue != null ? cellValue.ToString().Trim() : "";
                        }
                    }
                }
                ...
                foreach (DataGridViewRow LINHA in dataGridView1.Rows)
                {
                    if (LINHA.Cells[22].Value != null)
                    {
                        string opcao = LINHA.Cells[22].Value.ToString();
                        if (opcao == "Opção 2" || opcao == "Opção 5" || opcao == "Opção 6" || opcao == "Opção 16")
                        {
                            SOLDA = true;
                            break;
                        }
                ...
                if (SOLDA)
                {
                    ...
                    if (celula == null || !int.TryParse(celula.ToString(), out int numeroObra))
                    {
                        MessageBox.Show("Número da obra inválido ou ausente.");
                        return;
                    }

                    pdfitext.CriarPlanoSoldadura(
                        numeroObra.ToString("000"),
                        new Model().GetProjectInfo().Builder,
                        new Model().GetProjectInfo().ProjectNumber,
                        new Model().GetProjectInfo().Name,
                        "EXC" + EXC
                    );

                    string origem = "131 Lista de soldadores - CM.pdf";
                    string destino = "C:\\R\\131 Lista de soldadores - CM.pdf";
                    ...
                    {
                        MessageBox.Show("Arquivo PDF '131 Lista de soldadores - CM.pdf' não encontrado.");
                    }
                }
            }
            catch (Exception ex)
        ...
        //private void VerificarImportacaoPrimavera()
        //{
        //    string fase = null;
        //    string obra = lbl_numeroobra.Text;
        //    int ano = 0;
        //    string departamento = null;
        //    var primeiraLinha = dataGridView1.Rows
        //                         .Cast<DataGridViewRow>()
        //                         .Where(r => !r.IsNewRow && r.Cells.Count > 20)
        //                         .FirstOrDefault(r =>
        //                             r.Cells[0].Value != null && !string.IsNullOrWhiteSpace(r.Cells[0].Value.ToString()) &&
        //                             r.Cells[20].Value != null && !string.IsNullOrWhiteSpace(r.Cells[20].Value.ToString())
        //                         );
        ...
        //    departamento = primeiraLinha.Cells[20].Value?.ToString();
        //    fase = primeiraLinha.Cells[0].Value?.ToString().PadLeft(3, '0'); 
        ...
        //        string filtro = $"{user} -- {fase} -- *.txt";

        //        while (Directory.GetFiles(CaminhoCM, filtro).Length == 0)
        //        {
        //            DialogResult result = MessageBox.Show(this,
        //                $"A fase '{fase}' da obra '{obra}' não foi importada no Cegid/Primavera.",
        //                "Ficheiro RI - não encontrado",
        ...
        //    else
        //    {
        //        string Caminho = $@"\\Marconi\OFELIZ\OFELIZ\OFM\2.AN\2.CM\DP\11 Partilhada\{ano}\{departamento}\{obra}\{fase}";
        //        while (Directory.GetFiles(Caminho, "RI-*").Length == 0)
        //        {
        //            DialogResult result = MessageBox.Show(this,
        //                $"A fase '{fase}' da obra '{obra}' não foi importada no Cegid/Primavera'.",
        //                "Ficheiro RI- não encontrado",
