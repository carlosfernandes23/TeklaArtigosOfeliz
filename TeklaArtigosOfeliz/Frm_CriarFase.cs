...
namespace TeklaArtigosOfeliz
{
    public partial class Frm_CriarFase : Form
    {
        ...
        {
            LBLestado.Text = "A limpar numeração de peças e conjuntos...";
            try
            {
                ArrayList pecas = ComunicaTekla.ListadePecasdoConjSelec();
        ...
        private void button23_Click(object sender, EventArgs e)
        {
            ...
            if (VERIFICACAO)
            {
                if (label11.Text == m.GetProjectInfo().ProjectNumber)
                {
                    ...
                    TSM.Operations.Operation.CreateReportFromSelected("OFELIZPARAFUSOOBRA_Atualizado", @"C:\R\OFELIZ.CSV", "", "", "");
                    TSM.Operations.Operation.CreateReportFromSelected("OFELIZPARAFUSOOBRA_Atualizado_SemLotes_Sem_Data", @"C:\R\OFELIZ2.CSV", "", "", "");
        ...
        private void GrauprepeClasseex()
        {
            string Classeex = string.Empty;
            string Grauprep = string.Empty;
            try
            {
                ArrayList peças = new ArrayList(ComunicaTekla.ListadePecasdoConjSelec());
                ...
                while (index < peças.Count && !encontrouPrimeiraPeca)
                {
                    ...
                    if (peca != null)
                    {
                        ...
                        if (Grausuccess && Classeexsucc)
                        {
                            ...
                            string nobra = label11.Text;
                            string ano = "20" + nobra.Substring(0, 2);
                            string caminhoClasseEx = Path.Combine(
                            ...
                            try
                            {
                                if (Classeex == "2" || Classeex == "3")
                                {
                                    if (!File.Exists(caminhoClasseEx))
                                    {
                                        Directory.CreateDirectory(Path.GetDirectoryName(caminhoClasseEx));
                                        File.WriteAllText(caminhoClasseEx, Classeex.ToString());
                                    }
                                }

                                if (!File.Exists(caminhoGrau))
                                {
                                    Directory.CreateDirectory(Path.GetDirectoryName(caminhoGrau));
                                    File.WriteAllText(caminhoGrau, Grauprep.ToString());
                                }
                            }
                            catch (Exception ex)
                            {   }
