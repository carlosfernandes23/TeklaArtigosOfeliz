...
namespace TeklaArtigosOfeliz
{
    public partial class Frm_Parametros : Form
    {
        ...
        private void AtualizarBmecanicas()
        {
            string caminhoArquivo = @"\\marconi\OFELIZ\OFELIZ\OFM\2.AN\2.CM\DP\4 Produção\Desenvolvimentos\Ficheiros Temp tekla artigos (Nao Apagar)\BuchasMecanicas\BMecanicas.txt";

            if (File.Exists(caminhoArquivo))
            {
                string[] linhas = File.ReadAllLines(caminhoArquivo);
                comboBoxBMecanicas.Items.Clear();
                comboBoxBMecanicas.Items.AddRange(linhas);
            }
            else
            {
                MessageBox.Show("Arquivo de mecânicas não encontrado.");
            }

        }
        private void buttonBMecanicas_Click(object sender, EventArgs e)
        {
            string caminhoArquivo = @"\\marconi\OFELIZ\OFELIZ\OFM\2.AN\2.CM\DP\4 Produção\Desenvolvimentos\Ficheiros Temp tekla artigos (Nao Apagar)\BuchasMecanicas\BMecanicas.txt";
            string novaLinha = textBoxBMecanicas.Text.Trim();

            if (!string.IsNullOrWhiteSpace(novaLinha))
            {
                try
                {
                    bool precisaQuebra = false;
                    if (File.Exists(caminhoArquivo))
                    {
                        string ultimoConteudo = File.ReadAllText(caminhoArquivo);
                        if (!ultimoConteudo.EndsWith(Environment.NewLine))
                        {
                            precisaQuebra = true;
                        }
                    }
                    string textoParaAdicionar = (precisaQuebra ? Environment.NewLine : "") + novaLinha + Environment.NewLine;
                    File.AppendAllText(caminhoArquivo, textoParaAdicionar);
                    comboBoxBMecanicas.Items.Add(novaLinha);
                    textBoxBMecanicas.Clear();
            ...
            {
                MessageBox.Show("Por favor, insira uma mecânica no campo de texto.");
            }
        }

        private void button3_Click(object sender, EventArgs e)
        {
            string ComentarioBmecanica = comboBoxBMecanicas.Text.Trim();

            if (string.IsNullOrWhiteSpace(ComentarioBmecanica))
            {
                MessageBox.Show("Por favor, selecione ou insira um comentário para a buchas mecânicas.");
                return;
            }
            ...
            {
                MessageBox.Show("Tekla Structures não está conectado.");
                return;
            }

            ArrayList objetosSelecionados = new ArrayList();
            ArrayList guidsIncluidos = new ArrayList();

            Tekla.Structures.Model.UI.ModelObjectSelector selector = new Tekla.Structures.Model.UI.ModelObjectSelector();
            Tekla.Structures.Model.ModelObjectEnumerator enumerador = selector.GetSelectedObjects();

            while (enumerador.MoveNext())
            {
                Tekla.Structures.Model.Part part = enumerador.Current as Tekla.Structures.Model.Part;
                if (part != null)
                {
                    // Adiciona o conjunto (assembly)
                    Tekla.Structures.Model.Assembly assembly = part.GetAssembly();
                    if (assembly != null && !guidsIncluidos.Contains(assembly.Identifier.GUID))
                    {
                        guidsIncluidos.Add(assembly.Identifier.GUID);
                        objetosSelecionados.Add(assembly);
                    }

                    // Adiciona a própria peça
                    if (!guidsIncluidos.Contains(part.Identifier.GUID))
                    {
                        guidsIncluidos.Add(part.Identifier.GUID);
                        objetosSelecionados.Add(part);
                    }

                    // Adiciona parafusaria ligada à peça
                    Tekla.Structures.Model.ModelObjectEnumerator bolts = part.GetBolts();
                    while (bolts.MoveNext())
                    {
                        Tekla.Structures.Model.BoltGroup bolt = bolts.Current as Tekla.Structures.Model.BoltGroup;
                        if (bolt != null && !guidsIncluidos.Contains(bolt.Identifier.GUID))
                        {
                            guidsIncluidos.Add(bolt.Identifier.GUID);
                            objetosSelecionados.Add(bolt);
                        }
                    }
                }
                else if (enumerador.Current is Tekla.Structures.Model.BoltGroup boltSelecionado)
                {
                    if (!guidsIncluidos.Contains(boltSelecionado.Identifier.GUID))
                    {
                        guidsIncluidos.Add(boltSelecionado.Identifier.GUID);
                        objetosSelecionados.Add(boltSelecionado);
                    }
                }
                else if (enumerador.Current is Tekla.Structures.Model.Assembly assemblySelecionado)
                {
                    if (!guidsIncluidos.Contains(assemblySelecionado.Identifier.GUID))
                    {
                        guidsIncluidos.Add(assemblySelecionado.Identifier.GUID);
                        objetosSelecionados.Add(assemblySelecionado);
                    }
                }
            }
            ...
            {
                MessageBox.Show("Nenhuma peça válida selecionada no modelo.");
                return;
            }

            foreach (object obj in objetosSelecionados)
            {
                if (obj is Tekla.Structures.Model.Part part)
                {
                    part.SetUserProperty("BUM_COMMENT", ComentarioBmecanica);
                    part.Modify();
                }
                else if (obj is Tekla.Structures.Model.BoltGroup bolt)
                {
                    bolt.SetUserProperty("BUM_COMMENT", ComentarioBmecanica);
                    bolt.Modify();
                }
                else if (obj is Tekla.Structures.Model.Assembly assembly)
                {
                    assembly.SetUserProperty("BUM_COMMENT", ComentarioBmecanica);
                    assembly.Modify();
                }
            }
            ...
            MessageBox.Show(this,
                "Foram adicionados os requisitos a " + objetosSelecionados.Count + " objetos." + Environment.NewLine + ComentarioBmecanica,
                "INFORMAÇÃO",
        ...
        {
            this.Visible = false;
            Frm_EditarProRevestimentos F = new Frm_EditarProRevestimentos();
            F.ShowDialog();
            this.Visible = true;
        }
    }
